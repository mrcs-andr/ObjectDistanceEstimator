 ...
 * around its own X-axis by a known pitch angle. Given the pinhole intrinsics stored in
 * {@link CalibrationResult}, the bottom-centre of each detection's bounding box is back-projected
 * to a ray in world space. The intersection of that ray with the ground plane (world Y = 0) gives
 * the 3-D position of the foot of the object, and the Euclidean distance from the camera to that
 * point is returned in metres.</p>
 *
...
public class DistanceEstimator implements ILetterBoxObserver {
    ...
    public float estimate(Detection d) {
        ...
        float u = (u_m - lb.padX) / lb.scale;
        float v = (v_m - lb.padY) / lb.scale;
        ...
        // Normalised ray direction in camera frame
        double xn = (u - cx) / fx;
        double yn = (v - cy) / fy;
